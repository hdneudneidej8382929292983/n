...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if not is_bot:
        ...
        try:
            ...
            try:
                ...
                await two_step_msg.reply("» الباسورد غلط .\n\nقم بالاستخراج من جديد", quote=True, reply_markup=InlineKeyboardMarkup(gen_button))
                return
    else:
        if telethon:
    ...
    text = f"**جلستك يبروو {ty} ** \n\n`{string_session}` \n\n**تم الاستخراج بواسطة :** @Xx711e"
    try:
        if not is_bot:
